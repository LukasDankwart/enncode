...
class ONNXParser:
    ...
    def __init__(self, onnx_model_path):
        ...
        self.real_inputs_names = []
        self.real_output_names = []
        self._parse_model()
    ...
    def _parse_model(self):
        ...
        for initializer in self.graph.initializer:
            ...
            self.initializer_names.append(initializer.name)
        ...
        for input in self.graph.input:
            ...
            self.real_inputs_names.append(input.name)
        ...
        for output in self.graph.output:
            ...
            self.real_output_names.append(output.name)

        if not self.graph.input:
            raise ValueError("No input tensors to the ONNX model found.")

        self.current_shape = self.input_output_tensors_shapes[self.real_inputs_names[0]].copy()

        self.real_inputs_names = list(set(self.real_inputs_names))
        self.real_output_names = list(set(self.real_output_names))
    ...
    def get_input_output_tensors_shapes(self):
        """
        Simple getter function to get access when needed.

        Returns: input_output_tensor_shapes attribute of parser

        """
        return self.input_output_tensors_shapes
